...
func TestNewCycleList(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			head := NewCycleList(tc.inputVals, tc.cyclePos)

			// Handle the empty case first
			if len(tc.inputVals) == 0 {
			...
			nodes := make([]*ListNode, 0, len(tc.inputVals))
			vals := make([]int, 0, len(tc.inputVals))

			current := head
			for i := 0; i < len(tc.inputVals); i++ {
			...
			assert.Equal(t, tc.inputVals, vals, "The linear values of the list should be correct")

			// 2. Verify the cycle connection
			tailNode := nodes[len(nodes)-1]

			// If a cycle is expected
			if tc.cyclePos >= 0 && tc.cyclePos < len(tc.inputVals) {
