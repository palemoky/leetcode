...
2.  **向上取整 (Ceil) 技巧**：
    在分页 (Page), 分块 (Chunk)时，经常需要计算 $a/b$ 的 **向上取整** 结果（比如 10 条数据，每页 3 条，需要分 4 页）。相比于转换浮点数调用 `math.Ceil`，使用纯整数运算不仅 **性能更高**，且能彻底避免浮点数 **精度丢失** 的风险。
    ...
    | **防溢出公式**      | `(a - 1) / b + 1` | **无溢出风险** | $a=0$ 时结果错误 (算成 1)   |
