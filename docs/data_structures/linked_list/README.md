...
!!! Note "数组与链表的对比"
    ...
    **数组**像一排紧密排列的储物柜——柜子连续摆放，只要知道起始位置和编号，就能以 $O(1)$ 直接算出任意柜子的地址。

    **链表**则像一场寻宝游戏——每个藏宝点（节点）里都放着一张纸条，指引你前往下一个藏宝点。这些点散落在地图各处（内存非连续），你必须从起点出发、顺着线索一步步寻找，无法直接"空降"到目标位置。

## 核心概念

### 虚拟头节点

虚拟头节点(Dummy Node)是链表操作中最重要的技巧之一，主要用于链表的**头结点可能被修改，或者需要一个稳定的前驱节点**时。
...
| 条件                  | 访问范围                   | 循环结束时                 | 适用场景                               |
| :-------------------- | :------------------------- | :------------------------- | :------------------------------------- |
| `current != nil`      | 所有节点（包括最后一个）   | `current` 指向 `nil`         | 读取/处理每个节点的值                  |
...
1. 第 1 次抛硬币结果为正面：在 Level 1 建立索引
2. 第 2 次抛硬币结果为正面：在 Level 2 建立索引
3. 第 3 次抛硬币结果为反面：停止建立索引
