...
!!! question "为什么数组下标从 0 开始？"

    在绝大多数编程语言中，数组下标都从 0 开始（Lua、R、Matlab 等少数语言除外）。原因就藏在上面的寻址公式里——**下标的本质是"偏移量"**：

    - 第 1 个元素：偏移 0 步，地址 = 起始地址 → 下标为 **0**
    - 第 2 个元素：偏移 1 步，地址 = 起始地址 + 1 × 元素大小 → 下标为 **1**

    如果下标从 1 开始，每次寻址都要额外做一次 `index - 1` 的减法，既浪费了一次运算，也让公式失去了简洁性。从 0 开始，下标与偏移量天然统一，寻址一步到位。
