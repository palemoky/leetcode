...
> 关于遍历算法的详细讲解（BFS vs DFS、递归/迭代实现、四种解题模式等），请参考 [树的遍历算法](../../algorithms/traversal/README.md)。
...
!!! tip "堆的选择"
    ...
    - **Top-K**（反直觉）：求前 K 大用**小顶堆**，求前 K 小用**大顶堆**。堆顶是守门员，不够格的进不来。
    - **求极值**（直觉）：求最大值用大顶堆，求最小值用小顶堆，直接 peek。

如图所示，两张图分别用小顶堆和大顶堆维护了前 12 个最大值和最小值。以小顶堆为例，堆顶 `1` 就是守门员：插入 `0` 时，因 `0 < 1` 被直接拦截；插入 `10` 时，因 `10 > 1` 可以入堆，`1` 被弹出，`2` 成为新的堆顶守门员。
...
一个 B 树通过约束所有叶子节点在相同深度来保持平衡。深度在元素添加至树的过程中缓慢增长，而整体深度极少地增长，并导致所有叶子节点与根节点距离加 1。
...
Trie（发音 try，来自 re**trie**val）是一种专门用于高效处理字符串集合的多叉树结构，在搜索引擎的自动补全、拼写检查、IP 路由等场景中有广泛应用。
**核心特性：**
