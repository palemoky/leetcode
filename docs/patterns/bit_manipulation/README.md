...
| 运算符              | 名称            | 说明                        | 示例                                                  | 结果          |
| ------------------- | --------------- | --------------------------- | ----------------------------------------------------- | ------------- |
| `&`                 | 按位与（AND）   | 两位都为 1 时结果为 1（全真才真）       | `5 & 3` (`101 & 011`)                                 | `1` (`001`)   |
| <code>&#124;</code> | 按位或（OR）    | 任一位为 1 时结果为 1（有假则假）       | <code>5 &#124; 3</code> (<code>101 &#124; 011</code>) | `7` (`111`)   |
| `^`                 | 按位异或（XOR） | 两位不同时结果为 1（相同为 0，相异为 1）          | `5 ^ 3` (`101 ^ 011`)                                 | `6` (`110`)   |
| `~`                 | 按位取反（NOT） | 0 变 1，1 变 0（位反转）              | `~5` (`~0101`)                                        | `-6` (`1010`) |
| `<<`                | 左移            | 向左移动 n 位，右边补 0（指数级递增）     | `5 << 1` (`101 << 1`)                                 | `10` (`1010`) |
| `>>`                | 右移            | 向右移动 n 位，左边补符号位（指数级递减） | `5 >> 1` (`101 >> 1`)                                 | `2` (`010`)   |
...
| 最后一位变 1   | <code>n &#124; 1</code>     | 设置最低位为 1 |
