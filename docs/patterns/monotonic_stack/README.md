...
- 找下一个更高的人（递减栈）：栈为 `[180, 170, 160]`（底 → 顶），插入 175 时，依次弹出 160、170（均矮于 175），175 停在 180 旁。因为插入值 > 被弹出值，把挡路的人挤走，所以**插入值就是被弹出值的下一个更大的值**。
- 找下一个更矮的人（递增栈）：栈为 `[160, 170, 180]`（底 → 顶），插入 175 时，弹出 180（高于 175），175 停在 170 旁。因为插入值 < 被弹出值，把挡路的人挤走，所以**插入值就是被弹出值的下一个更小的值**。
