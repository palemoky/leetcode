...
从数据结构的演进来看，树可以理解为 **单向的多叉链表**。从图论的角度，树是特殊的图：**连通且无环的无向图**。

$$
\begin{array}{ccccc}
\textbf{链表} &  & \textbf{树} & & \textbf{图} \\
\text{线性结构} & \longrightarrow & \text{层次结构} & \longrightarrow & \text{网状结构} \\
\text{(一对一)} & & \text{(一对多)} & & \text{(多对多)}
\end{array}
$$
