...
前缀和（Prefix Sum）是一种预处理技巧，通过 **空间换时间** 的方式，将区间和查询的时间复杂度从 $O(n)$ 优化到 $O(1)$。

它特别适用于 **静态数组的频繁区间查询**，典型场景包括：

- **算法题**：区间求和（LC 303）、二维区域和（LC 304）、和为 K 的子数组。
- **业务开发**：
  - **历史数据挖掘**：快速计算过去任意时段的销售总额、PV/UV（前提是历史数据已归档，不再修改）。
  - **财务/股市**：计算账户历史累计盈亏、股票区间涨跌幅。
