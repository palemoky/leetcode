> 目标：**看到题目就知道用什么模板**

## 一、链表

### 基础必会

1. [**21** 合并两个有序链表](https://leetcode.cn/problems/merge-two-sorted-lists/) - 双指针基础
2. [**206** 反转链表](https://leetcode.cn/problems/reverse-linked-list/) - 必背模板
3. [**141** 环形链表](https://leetcode.cn/problems/linked-list-cycle/) - 快慢指针
4. [**19** 删除链表的倒数第 N 个结点](https://leetcode.cn/problems/remove-nth-node-from-end-of-list/) - 快慢指针进阶

### 进阶题

5. [**92** 反转链表 II](https://leetcode.cn/problems/reverse-linked-list-ii/) - 部分反转
6. [**142** 环形链表 II](https://leetcode.cn/problems/linked-list-cycle-ii/) - 数学推导
7. [**25** K 个一组翻转链表](https://leetcode.cn/problems/reverse-nodes-in-k-group/) ⭐ - 分组反转

## 二、二叉树

### 遍历类

1. [**102** 二叉树的层序遍历](https://leetcode.cn/problems/binary-tree-level-order-traversal/) - BFS 入门
2. [**144** 二叉树的前序遍历](https://leetcode.cn/problems/binary-tree-preorder-traversal/)
3. [**94** 二叉树的中序遍历](https://leetcode.cn/problems/binary-tree-inorder-traversal/)
4. [**145** 二叉树的后序遍历](https://leetcode.cn/problems/binary-tree-postorder-traversal/)

### 性质类

5. [**104** 二叉树的最大深度](https://leetcode.cn/problems/maximum-depth-of-binary-tree/)
6. [**101** 对称二叉树](https://leetcode.cn/problems/symmetric-tree/)
7. [**226** 翻转二叉树](https://leetcode.cn/problems/invert-binary-tree/)
8. [**112** 路径总和](https://leetcode.cn/problems/path-sum/)
9. [**110** 平衡二叉树](https://leetcode.cn/problems/balanced-binary-tree/)
10. [**98** 验证二叉搜索树](https://leetcode.cn/problems/validate-binary-search-tree/) ⭐ - BST 核心
11. [**543** 二叉树的直径](https://leetcode.cn/problems/diameter-of-binary-tree/) - 递归思维
12. [**236** 二叉树的最近公共祖先](https://leetcode.cn/problems/lowest-common-ancestor-of-a-binary-tree/) ⭐

## 三、数组 / 字符串

### 滑动窗口

1. [**3** 无重复字符的最长子串](https://leetcode.cn/problems/longest-substring-without-repeating-characters/)
2. [**438** 找到字符串中所有字母异位词](https://leetcode.cn/problems/find-all-anagrams-in-a-string/)
3. [**76** 最小覆盖子串](https://leetcode.cn/problems/minimum-window-substring/) ⭐
4. [**424** 替换后的最长重复字符](https://leetcode.cn/problems/longest-repeating-character-replacement/)
5. [**239** 滑动窗口最大值](https://leetcode.cn/problems/sliding-window-maximum/) - 单调队列

### 双指针

6. [**26** 删除有序数组中的重复项](https://leetcode.cn/problems/remove-duplicates-from-sorted-array/)
7. [**27** 移除元素](https://leetcode.cn/problems/remove-element/)
8. [**88** 合并两个有序数组](https://leetcode.cn/problems/merge-sorted-array/)
9. [**11** 盛最多水的容器](https://leetcode.cn/problems/container-with-most-water/)
10. [**15** 三数之和](https://leetcode.cn/problems/3sum/) ⭐ - 双指针经典

## 四、栈 & 队列

1. [**20** 有效的括号](https://leetcode.cn/problems/valid-parentheses/)
2. [**155** 最小栈](https://leetcode.cn/problems/min-stack/)
3. [**232** 用栈实现队列](https://leetcode.cn/problems/implement-queue-using-stacks/)
4. [**225** 用队列实现栈](https://leetcode.cn/problems/implement-stack-using-queues/)
5. [**739** 每日温度](https://leetcode.cn/problems/daily-temperatures/) - 单调栈入门

## 五、二分查找

1. [**704** 二分查找](https://leetcode.cn/problems/binary-search/)
2. [**35** 搜索插入位置](https://leetcode.cn/problems/search-insert-position/)
3. [**34** 在排序数组中查找元素的第一个和最后一个位置](https://leetcode.cn/problems/find-first-and-last-position-of-element-in-sorted-array/) ⭐
4. [**153** 寻找旋转排序数组中的最小值](https://leetcode.cn/problems/find-minimum-in-rotated-sorted-array/)
5. [**33** 搜索旋转排序数组](https://leetcode.cn/problems/search-in-rotated-sorted-array/)

## 六、回溯

1. [**78** 子集](https://leetcode.cn/problems/subsets/) - 回溯入门
2. [**77** 组合](https://leetcode.cn/problems/combinations/)
3. [**46** 全排列](https://leetcode.cn/problems/permutations/)
4. [**39** 组合总和](https://leetcode.cn/problems/combination-sum/)
5. [**47** 全排列 II](https://leetcode.cn/problems/permutations-ii/) - 去重技巧

## 七、动态规划

### 入门必做

1. [**70** 爬楼梯](https://leetcode.cn/problems/climbing-stairs/) - DP 入门
2. [**198** 打家劫舍](https://leetcode.cn/problems/house-robber/) - 状态转移
3. [**53** 最大子数组和](https://leetcode.cn/problems/maximum-subarray/)

### 经典题型

4. [**322** 零钱兑换](https://leetcode.cn/problems/coin-change/) - 完全背包
5. [**300** 最长递增子序列](https://leetcode.cn/problems/longest-increasing-subsequence/)
6. [**5** 最长回文子串](https://leetcode.cn/problems/longest-palindromic-substring/) - 区间 DP
7. [**1143** 最长公共子序列](https://leetcode.cn/problems/longest-common-subsequence/)

## 八、图论（BFS/DFS）

1. [**200** 岛屿数量](https://leetcode.cn/problems/number-of-islands/) - DFS/BFS 基础
2. [**133** 克隆图](https://leetcode.cn/problems/clone-graph/) - 图的遍历
3. [**207** 课程表](https://leetcode.cn/problems/course-schedule/) - 拓扑排序
4. [**994** 腐烂的橘子](https://leetcode.cn/problems/rotting-oranges/) - 多源 BFS

## 九、进阶高频（理解即可）

1. [**146** LRU 缓存](https://leetcode.cn/problems/lru-cache/) ⭐ - 哈希表 + 双向链表
2. [**208** 实现 Trie (前缀树)](https://leetcode.cn/problems/implement-trie-prefix-tree/)
3. [**124** 二叉树中的最大路径和](https://leetcode.cn/problems/binary-tree-maximum-path-sum/)
4. [**84** 柱状图中最大的矩形](https://leetcode.cn/problems/largest-rectangle-in-histogram/) - 单调栈
5. [**42** 接雨水](https://leetcode.cn/problems/trapping-rain-water/) ⭐ - 双指针/单调栈

---

## 推荐刷题节奏

### 每天 1 小时（5-6 题）

**周一至周五**（基础巩固）：

- 1 题链表/数组
- 1-2 题二叉树
- 1 题滑动窗口/双指针
- 1 题栈/队列/二分

**周末**（专题突破）：

- 3 题 DP（同一类型）
- 2 题回溯/图论

### 三遍刷题法

- **第 1 遍**：看题解，理解思路，能写对
- **第 2 遍**：不看答案，独立完成
- **第 3 遍**：边写边讲，能给别人讲清楚

### 进阶建议

- **一题多解**：尝试不同解法（暴力 → 优化 → 最优）
- **总结模板**：相同类型题目提炼通用模板
- **定期复习**：每周回顾本周错题
